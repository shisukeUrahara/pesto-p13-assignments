...
Test Case 1:
Sample Input: 1 2 3 -1 5 6 7

Sample Output: 18

Test Case 2:
sample input: 1 2 null 3 null 4 null 5

sample output: 15

Level: Easy
Hints:
Use a recursive approach to calculate the maximum path sum. At each node, compute the maximum path sum through that node (including the node itself) by considering both the left and right subtrees.
...
const maxPathSumRecursive = function (root) {
  if (!root) {
    return 0;
  }

  let leftPathSum = Math.max(maxPathSumRecursive(root.left), 0);
  let rightPathSum = Math.max(maxPathSumRecursive(root.right), 0);

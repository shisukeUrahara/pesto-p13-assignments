...
Input Format:
The number of eggs as an integer. The number of floors as an integer.
...
Level: Medium
Hints:
Use dynamic programming to solve this problem. Create a dynamic programming matrix dp to store the results. Base cases: If there are no floors, no attempts are needed. If there is only one floor, one attempt is needed. If there is only one egg, attempts needed are equal to the number of floors.
Fill the rest of the matrix using dynamic programming: For each number of eggs and floors, calculate the minimum number of attempts needed. Iterate through all possible floors to drop the egg. Determine the maximum attempts needed among the two possibilities: egg breaks or egg doesn't break. Take the minimum of these maximum attempts. Return dp[eggs][floors], which contains the minimum number of attempts.
Approach:
...
function eggDropOptimized(eggs, floors) {
  let dp = Array.from({ length: eggs + 1 }, () => Array(floors + 1).fill(0));
...
// Test cases
console.log(eggDropOptimized(2, 10)); // Output: 4
console.log(eggDropOptimized(3, 14)); // Output: 4

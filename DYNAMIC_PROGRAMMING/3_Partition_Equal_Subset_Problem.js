...
Level: Medium
Hints:
Use dynamic programming to solve this problem. Calculate the total sum of the numbers in the set. If the total sum is odd, it cannot be divided into two equal subsets, so return false. Calculate the target sum for each subset, which is half of the total sum. Create a dynamic programming array dp to store the results. Initialize dp[0] as true since an empty subset can have a sum of 0.
Iterate through each number in the set. Iterate from the target sum down to the current number. Check if including the current number can reach the current sum. After the iterations, dp[targetSum] will contain the result for the target sum. Return dp[targetSum].
Approach:
...
// Test cases
console.log(partitionEqualSubset([1, 5, 11, 5, 3])); // Output: false
console.log(partitionEqualSubset([1, 2, 3, 4, 5, 6, 7])); // Output: true

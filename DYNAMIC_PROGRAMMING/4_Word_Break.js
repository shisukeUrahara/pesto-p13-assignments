...
Input Format:
The user is prompted to enter the string. The user is prompted to enter the dictionary words as a space-separated string.

Output Format:
The program prints whether the string can be segmented into dictionary words or not.

Test Case 1:
Sample Input: applepenapple apple pen

Sample Output: true

Test Case 2:
sample input: pineapplepenapple apple pen pineapple

sample output: true

Level: Medium
Hints:
Use dynamic programming to solve this problem. Create a dynamic programming array dp to store the results. Initialize dp[0] as true since the empty string is always present in the dictionary. Iterate through each character of the string and check if the substring up to the current character can be segmented.
Use a nested loop to check all possible substrings and update dp[i] if a matching word is found in the dictionary. After the iterations, dp[s.length] will contain the result for the entire string. Return dp[s.length].
Approach:
...
console.log(wordBreak("pineapplepenapples", ["apple", "pen", "pineapple"])); // Output: true

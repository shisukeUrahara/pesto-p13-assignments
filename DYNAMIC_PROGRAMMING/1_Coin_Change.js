...
Input Format:
The user is prompted to enter the coin denominations as a space-separated string of integers. The user is prompted to enter the target amount asa single integer.
...
Level: Easy
Hints:
Use dynamic programming to solve this problem. Create an array dp to store the minimum number of coins needed for each amount. Initialize dp[0] as 0 and all other elements as Infinity. Iterate through each amount from 1 to the target amount.
For each amount, iterate through each coin denomination. If the coin denomination is less than or equal to the current amount, check if using that coin denomination results in a smaller number of coins than the current minimum. If so, update the minimum. After the iterations, dp[amount] will contain the minimum number of coins required for the target amount. Return dp[amount] if it is not Infinity, otherwise return -1.
Approach:
...
// Example usage
console.log(coinChange([1, 2, 5, 11], 11)); // Output: 1
console.log(coinChange([2, 3], 1)); // Output: -1

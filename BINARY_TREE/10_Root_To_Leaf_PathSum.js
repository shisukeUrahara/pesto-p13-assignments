...
Test Case 1:
Sample Input: 5 4 8 11 null 13 4 7 2 null null null 1 22
...
Level: Hard
Hints:
Traverse the tree in a depth-first manner, keeping track of the current sum. At each node, subtract the node's value from the current sum. Check if the current node is a leaf node and if the current sum is zero. If both conditions are satisfied, return true.
Recursively check for the left and right subtrees, passing the updated sum. If none of the paths result in a sum of zero at a leaf node, return false.
Approach:
...
*/
const pathSumRecursive = function (root, sum, targetSum) {
  if (!root) {
    return false;
  }
  ...
  return (
    pathSumRecursive(root.left, sum, targetSum) ||
    pathSumRecursive(root.right, sum, targetSum)
  );
};
var hasPathSum = function (root, targetSum) {
  let sum = 0;
  return pathSumRecursive(root, sum, targetSum);
};

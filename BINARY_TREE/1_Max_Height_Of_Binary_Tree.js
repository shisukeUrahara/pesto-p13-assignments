...
Test Case 2:
sample input: 10 5 20 3 8 null 25 null null null null 7

sample output: 4

Level: Easy
Hints:
The height of an empty tree is 0. The height of a tree can be calculated recursively by finding the maximum height of its left and right subtrees.
Each recursive call will return the height of the subtree rooted at the current node. The height of the overall tree is the maximum height among the left and right subtrees, plus 1.
Approach:
...
function getMaxDepth(depth, node) {
  ...
  return Math.max(
    getMaxDepth(depth + 1, node.left),
    getMaxDepth(depth + 1, node.right)
  );
}
...
var maxDepth = function (root) {
  ...
  return Math.max(
    getMaxDepth(depth, root.left),
    getMaxDepth(depth, root.right)
  );
};
